...
# Creating a tibble with different data types for each column
example_tibble <- tibble(
  col_chr     = c("apple", "banana", "grape", "N/A", "", "kiwi", "melon", "berry", "peach", "plum"),  # Character column
  `col$num`     = c(1.1, 2.5, -9999, 4.2, 5.9, 6.3, 7.7, 8.1, NA, 10.5),                                        # Numeric column
  col_log     = c(TRUE, FALSE, TRUE, TRUE, FALSE, NA, FALSE, TRUE, FALSE, TRUE),                           # Logical column
  col_date    = as.Date(c("2024-01-01", "2024-01-05", "2024-01-10", "2024-01-15",
                          "2024-01-20", "2024-01-25", "2024-01-30", "2024-02-05",
                          "2024-02-10", "2024-02-15")),                                                      # Date column
  col_mixed   = as.character(c(10, "twenty", 30, "forty", 50, "N/A", 70, "eighty", 90, "one hundred")),    # Mixed data (numeric and text)
  col_datetime = ymd_hms(c("2024-01-01 12:34:56", "2024-01-05 06:12:43", NA,
                           "2024-01-15 18:45:12", "2024-01-20 23:59:59", "2024-01-25 08:30:15",
                           "2024-01-30 09:25:32", "2024-02-05 21:16:18", "2024-02-10 11:11:11",
                           "2024-02-15 16:45:50")),                                                           # Date-time column
  col_time123    = as_hms(c("12:00:00", "06:15:45", "14:30:25", "18:00:00", "23:45:00", "08:05:15",
                            "09:15:00", "21:45:25", "11:59:59", "16:10:00"))                                        # Time-only column
)
...
test_that("required file strings are present", {
  ...
  file_strings <-  list(flmd = ".*flmd\\.csv$", # file that ends with flmd.csv
  ...
  expect_equal(check_for_required_file_strings(input = c("example.csv", "example_flmd.csv", "readme_example.pdf", "example_dd.csv"), required_file_strings = file_strings),
  ...
  expect_equal(check_for_required_file_strings(input = c("example.csv", "example2.csv", "example3.csv"), required_file_strings = file_strings),
               tribble(~requirement, ~pass_check, ~assessment, ~input, ~value, ~source, ~file,
  ...
  expect_equal(check_for_required_file_strings(input = c("example.csv", "example_flmd.csv", "example_dd.csv"), required_file_strings = file_strings),
               tribble(~requirement, ~pass_check, ~assessment, ~input, ~value, ~source, ~file,
                       "required", TRUE, "includes required files", "example_flmd.csv", ".*flmd\\.csv$", "all_file_names", "example_flmd.csv",
                       "required", TRUE, "includes required files", "example_dd.csv", ".*dd\\.csv$", "all_file_names", "example_dd.csv",
                       "required", FALSE, "includes required files", NA_character_, "^readme.*\\.pdf$", "all_file_names", NA_character_))
  ...
  expect_equal(check_for_required_file_strings(input = c("example.csv", "example_flmd.csv", "example_readme.pdf", "example_dd.csv"), required_file_strings = file_strings),
               tribble(~requirement, ~pass_check, ~assessment, ~input, ~value, ~source, ~file,
                       "required", TRUE, "includes required files", "example_flmd.csv", ".*flmd\\.csv$", "all_file_names", "example_flmd.csv",
                       "required", TRUE, "includes required files", "example_dd.csv", ".*dd\\.csv$", "all_file_names", "example_dd.csv",
                       "required", FALSE, "includes required files", NA_character_, "^readme.*\\.pdf$", "all_file_names", NA_character_))
