...
overview_table %>% 
  ...
  mutate(failed_files = cell_spec(failed_files, background = case_when(failed_files > 0 ~ "lightcoral",
                                                                       T ~ ""))) %>% 
  kable("html", escape = FALSE, row.names = TRUE, align = c("c", "l", "c", "c", "c")) %>% 
  kable_styling("striped", full_width = FALSE, position = "center") %>% 
  row_spec(which(overview_table$pass_check == "PASSED"), color = "gray") %>% 
  row_spec(which(overview_table$pass_check == "FAILED"), color = "black", bold = T) %>% 
  pack_rows("Required Checks", 1, sum(overview_table$requirement == "required"), label_row_css = "background-color: #e6e6e6; color: #000000;") %>%
  pack_rows("Strongly Recommended Checks", 
            sum(overview_table$requirement == "required") + 1, 
            nrow(overview_table), label_row_css = "background-color: #e6e6e6; color: #000000;")
...
#   mutate(failed_files = cell_spec(failed_files, background = case_when(failed_files > 0 ~ "lightcoral",
#                                                                        T ~ ""))) %>% 
#   kable("html", escape = FALSE, row.names = TRUE) %>% 
#   kable_styling("striped", full_width = FALSE, position = "center") %>% 
#   row_spec(which(overview_table$pass_check == "PASSED"), color = "gray") %>% 
#   row_spec(which(overview_table$pass_check == "FAILED"), color = "black", bold = T)
...
``` {r echo = FALSE}
tribble(~file, ~`file naming requirement`, ~`file name example`,
        ...
        "FLMD", "File must end with `flmd.csv`. You may prepend anything to the beginning, but we recommend the text matches your data package folder name.", "`Smith_2005_Wildfire_Impacts_flmd.csv`", 
        "DD", "File must end with `dd.csv`. You may prepend anything to the beginning, but we recommend the text matches your data package folder name.", "`Smith_2005_Wildfire_Impacts_dd.csv`", 
        "ESS-DIVE metadata", "This 4th file needs to be filled out to populate your data package landing page on ESS-DIVE; however, because the file will not be included in your data package there is not a check for it.", "N/A") %>% 
...
> <p style='color: `r ifelse(failed_no_special_chrs > 0, "darkred", "green")`;'><strong> **Your data package has `r failed_no_special_chrs` folder/file/column name(s) that contain special characters.**  </strong></p>  
