...
- **`action`** = Specifies the action or change required to address the issue identified by the assessment, including instructions on how to resolve the issue.  
...
# option 1 - shorter assessment text
data_package_checks$data_checks %>% 
    filter(pass_check == FALSE) %>%
    rowwise() %>% 
...
# option 2 - more user friendly assessment text
data_package_checks$data_checks %>% 
    filter(pass_check == FALSE) %>%
    rowwise() %>% 
