...
directory <- "Z:/00_ESSDIVE/01_Study_DPs/SFA_SpatialStudy_2021_SampleData_v3/v3_SFA_SpatialStudy_2021_SampleData" 
...
# 1. Load data. TIP: to exclude/include multiple files, go to the directory > select all > copy path. Paste within c(). Click alt + shift + drag cursor over all lines > click "End" on keyboard > add a comma.
...
prelim_dd <- read_csv("Z:/00_ESSDIVE/01_Study_DPs/SFA_SpatialStudy_2021_SampleData_v3/v2_SPS_Sample_dd_OLD.csv")
...
prelim_flmd <- read_csv("Z:/00_ESSDIVE/01_Study_DPs/SFA_SpatialStudy_2021_SampleData_v3/v3_SPS_Sample_flmd_OLD.csv") %>%
  select(-File_Path)
