...
summary <- combine_remove_outliers %>% 
  select(-extract_lookup_text) %>% 
  group_by(parent_id, file_name) %>% 
  mutate(average = mean(data_value, na.rm = T)) %>% 
  ...
  select(Sample_Name, Material, average, summary_header_name) %>%  # drop cols we don't need anymore
  distinct() %>% 
  pivot_wider(id_cols = c(Sample_Name, Material), names_from = summary_header_name, values_from = average)
