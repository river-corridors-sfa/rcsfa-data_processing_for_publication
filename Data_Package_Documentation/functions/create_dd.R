...
create_dd <- function(files_df, 
  ...
  if (add_flmd_dd_headers == T) {
    ...
    dd_placeholder_entires <- tribble(~Column_or_Row_Name, ~Unit, ~ Definition, ~Data_Type, ~Term_Type, ~Missing_Value_Code, ~associated_files, 
                                      ...
                                      "Term_Type", "N/A", "Indicates how the term is used in the data package (e.g., a column header, row header, data flag, or other variable).", "text", "column header", '"N/A"; "-9999"; ""; "NA"', "dd template", 
