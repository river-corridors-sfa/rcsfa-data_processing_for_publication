### HJW_rename_and_copy_raw_FTICR_folders.R #################################
# Date Created: 2024-10-29
# Date Updated: 2024-11-19
# Author: Bibi Powers-McCormack

# Objective: 
  # use the HJW mapping files to create rename and move .d folders

# Assumptions: 
  # this only copies HJW files
...
directory <- "Z:/Large_File_Storage_For_Raw_Instrument_Data/UA FTICR Bruker SolariX 9T/SHIPDATE20260407_Data_Raw_ICR_HJW_E1-287"

xml_file_names <- list.files(directory, pattern = "\\.xml$", recursive = T, full.names = T) %>% 
  tibble(file_path = .)  # yes, 285 xmls present
...
source_mapping_file <- readxl::read_excel("C:/Users/forb086/OneDrive - PNNL/RC-SFA - Documents/Study_HJW/FTICR/01_RawData/20260407_Data_Raw_FTICR_HJW_E1/SHIPDATE20260407_Mapping_Raw_ICR_HJW_E1-287.xlsx",
                                          skip = 1)
...
# drop rows that say OMIT in the Notes column and those that are missing an ion accumulation time 
  ...
  filter(
    (is.na(Method_Notes) | !str_detect(Method_Notes, "\\bOMIT\\b")) &
      (is.na(Method_Deviation) | !str_detect(Method_Deviation, "ICR_005"))
  )%>%
  ...
  mutate(analyte_subdir = case_when(str_detect(Sample_ID, "ICR") ~ "Water",
                                    str_detect(Sample_ID, "SIR") ~ "Sediment",
                                    str_detect(Sample_ID, "Blk") ~ "Blank")) %>% 
...
source_mapping_file %>%
  filter(
    is.na(Accumulation_Time) |
      str_detect(coalesce(Method_Notes, ""), "\\bOMIT\\b") |
      str_detect(coalesce(Method_Deviation, ""), "ICR_005")
  )
...
# create directories
in_dir <- directory
out_dir <- "Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/WHONDRS_HJW_FTICR_Raw_Data"

# list all folders
source_dirs <- list.dirs(in_dir, recursive = F, full.names = T) %>% # get all parent folders
  .[str_detect(., "HJW")]

# not needed for HJW because there is only one folder
# %>%  # filter for parent (SHIP) files that have HJW in them
#   list.dirs(., recursive = F) %>% # get all sub folders from those parent dirs
#   str_subset(., "/HJW_")  # filter those for HJW

# convert folders into df
source_dirs_df <- tibble(source = source_dirs) %>% 
  mutate(source_folder = basename(source),
         match_name = str_c(
           str_extract(source_folder, "HJW_E\\d+"),
           ".d"
         ))
...
source_dirs_df %>% 
  anti_join(mapping_file, by = join_by(match_name == randomized_id_dot_d)) # the poorly resolved data will not be included


# show the files that are in the filtered mapping file but we don't have a folder
mapping_file %>% 
  anti_join(source_dirs_df, by = join_by(randomized_id_dot_d == match_name)) # none
  

# create look up df 
HJW_lookup_df <- mapping_file %>% # uses mapping file as source of truth for which files to move
  
  # join mapping file
  left_join(source_dirs_df, by = join_by(randomized_id_dot_d == match_name)) %>% 
...
test_that("All sediment samples are present", {
  ...
  boye <- read_csv("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/v2_WHONDRS_HJW_Data_Package/v2_WHONDRS_HJW_Sample_Data/WHONDRS_HJW_Sediment_FTICR_Methods.csv", skip = 2, na = c("N/A", "-9999")) %>%
  ...
  sed_filter <- HJW_lookup_df %>%
    filter(str_detect(Sample_ID, "SIR")) %>% 
    arrange(Sample_ID)
  
  # compares the boye sample names with those in the lookup 
  expect_equal(sort(boye$Sample_Name), sort(sed_filter$Sample_ID))
  
  # join look up to boye to look if there are issues
  boye_lookup_join <- boye %>% 
    left_join(HJW_lookup_df, by = join_by(Sample_Name == Sample_ID))
  
  boye_lookup_join <- boye %>%
    select(Sample_Name) %>%
    add_column(Sample_ID = sed_filter$Sample_ID) %>%
    mutate(check = Sample_Name == Sample_ID)
...
test_that("All water samples are present", {
  ...
  boye <- read_csv("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/v2_WHONDRS_HJW_Data_Package/v2_WHONDRS_HJW_Sample_Data/WHONDRS_HJW_Water_FTICR_Methods.csv", skip = 2, na = c("N/A", "-9999")) %>%
    filter(!is.na(Sample_Name)) %>%
    filter(Sample_Name != "") %>% 
    filter(!is.na(`FTICR-MS`)) %>% 
    arrange(Sample_Name)
...
# clean up 
HJW_lookup_df <- HJW_lookup_df %>% 
  select(source, destination)


### Run function ###############################################################

rename_and_copy_folders(HJW_lookup_df)
...
test_that("folders were correctly copied and renamed", {
  ...
  raw_water_files <- list.files("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/WHONDRS_HJW_FTICR_Raw_Data/Water_FTICR_Raw_Data") %>%
    tibble() %>%
    mutate(across(where(is.character), ~ gsub("\\.d", "", .))) %>% 
    pull(.) %>% 
    sort()
  
  raw_sed_files <- list.files("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/WHONDRS_HJW_FTICR_Raw_Data/Sediment_FTICR_Raw_Data") %>%
    tibble() %>%
    mutate(across(where(is.character), ~ gsub("\\.d", "", .))) %>% 
    pull(.) %>% 
    sort()
  
  raw_blk_files <- list.files("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/WHONDRS_HJW_FTICR_Raw_Data/Blank_FTICR_Raw_Data") %>%
    tibble() %>%
    mutate(across(where(is.character), ~ gsub("\\.d", "", .))) %>% 
    pull(.) %>% 
    sort()
  
  xml_water_files <-  list.files("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/v2_WHONDRS_HJW_Data_Package/v2_WHONDRS_HJW_Sample_Data/FTICR/Water_XML_Files") %>%
    tibble()%>%
    mutate(across(where(is.character), ~ gsub("\\.xml", "", .))) %>% 
    pull(.) %>% 
    sort()
  
  xml_sed_files <- list.files("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/v2_WHONDRS_HJW_Data_Package/v2_WHONDRS_HJW_Sample_Data/FTICR/Sediment_XML_Files") %>%
    tibble() %>%
    mutate(across(where(is.character), ~ gsub("\\.xml", "", .))) %>% 
    pull(.) %>% 
    sort()
  
  xml_blk_files <- list.files("Z:/00_ESSDIVE/01_Study_DPs/WHONDRS_HJW_Data_Package_v2/v2_WHONDRS_HJW_Data_Package/v2_WHONDRS_HJW_Sample_Data/FTICR/Blank_XML_Files") %>%
    tibble() %>%
    mutate(across(where(is.character), ~ gsub("\\.xml", "", .))) %>% 
    pull(.) %>% 
    sort()
  ...
  expect_equal(xml_blk_files, raw_blk_files)
